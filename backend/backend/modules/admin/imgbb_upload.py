...
async def tgm_cmd(_, message: types.Message) -> None:
    ...
    try:
        ...
        if remote_url:
            ...
            await status_msg.edit_text(f"❌ <b>Failed to securely host the media on Catbox or ImgBB.</b>", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
        LOGGER.error(f"Error in tgm_cmd: {e}")
        await status_msg.edit_text(f"❌ <b>An error occurred during upload.</b>", parse_mode=enums.ParseMode.HTML)
