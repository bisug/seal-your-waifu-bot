...
def clean_text(text: str) -> str:
    ...
    text = re.sub(r'\[.*?\]', '', text)
    # Remove parenthetical counts like (x1) or (1/89)
    text = re.sub(r'\((?:x\s*)?\d+(?:/\d+)?\)', '', text, flags=re.I)
    # Strip trailing symbols/emojis to reveal IDs
    text = re.sub(r'[^\w\s\.]+$', '', text)
    # Remove trailing counts "x10" or IDs (3+ digits); \b keeps "Area 51" intact
    text = re.sub(r'\s+x\d+\s*$', '', text, flags=re.I)
    text = re.sub(r'\s+\d{3,}\s*$', '', text)
    # ID stuck to the name ("AnimeName12345") — only clearly long IDs (5+ digits)
...
async def scrape_group_command_handler(client, message):
    ...
    # Numeric IDs (incl. negative for groups/channels)
    try:
    ...
    try:
        # Userbot required for private groups; bot works for public ones.
        ...
        try:
            async for msg in client_to_use.get_chat_history(chat.id, limit=scan_limit):
                ...
                # pending_characters: session-only fast-path cache;
                # scraped_characters_collection: authoritative dedup across restarts.
                ...
                temp_path = None
                try:
                    # Download first so we have a valid file to re-upload.
...
async def approve_scrape_callback(client, query):
    ...
    # Strip any trailing IDs that were in the review message
    ...
    temp_path = None
    try:
        # Message object (not .photo) — scraped items may be documents.
        # Absolute dir: kurigram 2.2.25 resolves relative paths against workdir.
...
async def decline_scrape_callback(client, query):
    ...
    if name_match and anime_match:
        ...
        anime = anime_match.group(1).strip()
        key = (name.lower(), anime.lower())
        # Not removed from scraped_characters_collection: declined items
        # must not be re-sent.
        if key in pending_characters:
            pending_characters.remove(key)
