...
const getDisplayName = (user: LeaderboardUser, index: number) => {
    ...
    return `Operator ${String(user.id || index + 1).slice(-4).toUpperCase()}`;
};
                ...
                <AnimatePresence mode="wait">
                    {error && !data ? (
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
                            {data.slice(0, visible).map((user, i) => {
                                ...
                                <Card key={user.id} variant="default" className={cn(
                                    ...
                                    rank === 1 ? "border-amber-500/30 bg-amber-500/[0.02]" :
                                    rank === 2 ? "border-zinc-400/20" :
                                    ...
                                    <div className="flex items-center gap-4 min-w-0">
                                        <div className={cn(
                                            ...
                                            rank === 2 ? 'bg-zinc-400/20 text-zinc-300 border-zinc-400/30' :
                                        ...
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div className="min-w-0 space-y-0.5">
                                                ...
                                                <p className="text-[9px] font-bold text-zinc-600 truncate uppercase tracking-widest leading-none">
                                                    {user.username ? `@${user.username}` : `ID ${String(user.id).slice(0, 8)}`}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="text-right shrink-0 pl-4 space-y-1">
                                        <div className="flex items-center justify-end gap-2 h-7 px-2.5 rounded bg-zinc-900 border border-white/5">
                                            <span className="text-[11px] font-mono font-bold text-zinc-100 tabular-nums">{formatNumber(user.value)}</span>
                                            {activeMetric && <activeMetric.icon size={11} className={cn(
                                                rank === 1 ? 'text-amber-500' : rank === 2 ? 'text-zinc-400' : rank === 3 ? 'text-amber-700' : 'text-zinc-600'
                                            )} />}
                                        </div>
                                        <p className="text-[8px] font-bold text-zinc-700 uppercase tracking-widest">{activeMetric?.label || metric}</p>
                                    </div>
                                </Card>
                                );
                            })}
                        ...
                        <div className="py-20 border border-dashed border-white/5 rounded-lg bg-zinc-950/50 text-center flex flex-col items-center justify-center space-y-3">
                            ...
                            <p className="text-[9px] font-bold text-zinc-700 uppercase tracking-widest">No ranking data</p>
                        </div>
                    )}
                </AnimatePresence>
            </div>
