  ...
  return (
    <div className="pb-32 pt-6 max-w-5xl mx-auto adaptive-px space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* User Profile Card */}
        <Card variant="surface" className="md:col-span-2 flex flex-col sm:flex-row items-center gap-6 p-6">
          ...
          <div className="flex-1 min-w-0 space-y-3 text-center sm:text-left">
            <div>
                ...
                <p className="text-[10px] font-mono font-medium text-zinc-500 uppercase tracking-widest">{usernameLabel}</p>
            </div>
        ...
        <Card variant="surface" className="p-6 flex flex-col justify-between">
           <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Archive</span>
              </div>
              <Badge variant="secondary" size="xs">Registry</Badge>
           </div>

           <div className="mt-6 space-y-4">
              <div className="flex items-end justify-between">
                 <div className="text-3xl font-mono font-bold text-zinc-100 tabular-nums leading-none">
                    {collectionPercent}<span className="text-sm text-zinc-500 ml-1">%</span>
                 </div>
                 <div className="text-right">
                    <div className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mb-1">COLLECTED</div>
        ...
        {[
          ...
          <Card key={i} variant="default" className="p-4 group">
            ...
            <div className="flex flex-col">
               ...
               {stat.subValue && (
                 <span className="text-[8px] font-bold text-zinc-600 uppercase tracking-wider mt-1">{stat.subValue}</span>
               )}
            </div>
          </Card>
        ))}
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card variant="default" className="flex items-center gap-4 p-4 hover:bg-zinc-900 transition-colors cursor-pointer group">
           <div className="w-10 h-10 rounded bg-zinc-900 border border-white/5 flex items-center justify-center shrink-0">
              <PawPrint size={18} className="text-zinc-400 group-hover:text-brand-accent transition-colors" />
           </div>
           <div className="min-w-0 flex-1">
              <div className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5">COMPANION</div>
              <div className="text-xs font-bold text-zinc-100 truncate uppercase tracking-tight">{activePet?.name || 'NONE ACTIVE'}</div>
           </div>
           {activePet && (
              <Badge variant="secondary" size="xs" className="font-mono">LVL {activePet.level || 1}</Badge>
           )}
        </Card>

        <Card variant="default" className="flex items-center gap-4 p-4 hover:bg-zinc-900 transition-colors cursor-pointer group">
           <div className="w-10 h-10 rounded bg-zinc-900 border border-white/5 flex items-center justify-center shrink-0">
              <Egg size={18} className="text-zinc-400 group-hover:text-emerald-500 transition-colors" />
           </div>
           <div className="min-w-0 flex-1">
              <div className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest mb-0.5">INCUBATOR</div>
      ...
      <section ref={collectionRef} className="pt-4 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
            <div className="space-y-1">
                <h2 className="text-xl font-bold text-zinc-100 uppercase tracking-tight">Archive Registry</h2>
                <div className="flex items-center gap-1.5">
                    <Zap size={10} className="text-brand-accent" />
                    <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-widest">Synchronized with protocol node</p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="relative">
                    ...
                    <ChevronDown size={12} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-zinc-600 pointer-events-none" />
                </div>
            </div>
        </div>
