...
const QuestItem = ({ quest, onComplete, completing }) => (
  <div key={quest.id} className={`glass-panel p-6 rounded-[2.5rem] border transition-all ${quest.completed ? 'border-brand-accent/20 bg-brand-accent/[0.02]' : 'border-white/5'}`}>
      <div className="flex justify-between items-start mb-5">
          <div className="flex-1">
              <h3 className="text-sm font-black text-white uppercase italic tracking-tight mb-1">{quest.title}</h3>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{quest.description}</p>
          </div>
          <div className="flex flex-col items-end">
              <span className="text-[13px] font-black text-brand-accent">⧫ {formatNumber(quest.reward)}</span>
              <span className="text-[7px] font-black text-slate-600 uppercase tracking-widest">Reward</span>
          </div>
      </div>
      ...
      <div className="flex items-center gap-4">
          <div className="flex-1">
              ...
              <div className="h-1.5 bg-black/40 rounded-full overflow-hidden">
                  <div
                      className={`h-full transition-all duration-1000 ${quest.completed ? 'bg-brand-accent' : 'bg-brand-accent/50'}`}
                      style={{ width: `${Math.min(100, (quest.current / quest.total) * 100)}%` }}
                  />
              </div>
          </div>
          ...
          {quest.completed ? (
              ...
              <button
                  ...
                  disabled={quest.current < quest.total || completing === quest.id}
...
export const Quests = () => {
    ...
    const handleComplete = async (questId) => {
        ...
        try {
            const res = await apiFetch(`/quests/complete/${questId}`, { method: 'POST' });
            addToast(`QUEST SECURED: +${res.reward} ZENITH`, 'success');
    ...
    return (
        <div className="pb-32 pt-6">
            <header className="px-6 mb-8">
                ...
                <p className="text-[10px] font-bold text-brand-accent uppercase tracking-[0.4em] mt-1">Complete tasks to earn Zenith</p>
            </header>
