...
import { CheckCircle2, ClipboardList, Target, Lock } from 'lucide-react';
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2.5 mb-1.5">
                        <div className={cn(
                            ...
                            isComplete ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" : "bg-zinc-900 border-white/5 text-brand-accent"
                        )}>
                            <Target size={14} />
                        </div>
                        <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-tight truncate">{quest.name}</h3>
                    </div>
                    <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest line-clamp-1 pl-0.5">{quest.description}</p>
                </div>
                <div className="flex flex-col items-end shrink-0">
                    <div className="flex items-center gap-1.5 h-6 px-2 rounded bg-zinc-900 border border-white/5">
                        <span className="text-[10px] font-mono font-bold text-zinc-100 leading-none">{formatNumber(quest.reward_shards)}</span>
                        <span className="text-[8px] font-bold text-amber-500 uppercase tracking-widest leading-none">Shards</span>
                    </div>
        ...
        <section className="space-y-4">
            ...
            {quests.length > 0 ? (
                ...
                <div className="py-12 border border-dashed border-white/5 rounded-lg bg-zinc-950/50 text-center flex flex-col items-center justify-center space-y-2">
                    <p className="text-[9px] font-bold text-zinc-700 uppercase tracking-widest">No Missions Available</p>
                </div>
            )}
        </section>
    );
