...
const LeaderboardItem = memo(({ user, index, metric, getMetricIcon }) => (
  <motion.div
    ...
    className={`glass-panel p-4 rounded-2xl border flex items-center space-x-4 ${
      index === 0 ? 'border-brand-accent bg-brand-accent/5' : index === 1 ? 'border-brand-accent/30' : index === 2 ? 'border-amber-400/30' : 'border-white/5'
    }`}
  >
    ...
    <div className="relative">
       ...
       {index === 0 && <div className="absolute -top-1 -right-1 w-3 h-3 bg-brand-accent rounded-full animate-ping" />}
    </div>
