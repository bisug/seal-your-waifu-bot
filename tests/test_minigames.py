...
from Grabber.core.minigames import get_user_energy, consume_energy, MAX_ENERGY, RECHARGE_MINUTES
...
async def test_energy_recharge():
    ...
    with patch("Grabber.core.minigames.user_collection.update_one", AsyncMock()) as mock_update:
        with patch("Grabber.core.minigames.invalidate_user_cache", AsyncMock()):
...
async def test_consume_energy_success():
    ...
    with patch("Grabber.core.minigames.user_collection.find_one", AsyncMock(return_value=user_data)):
        with patch("Grabber.core.minigames.user_collection.update_one", AsyncMock()) as mock_update:
            with patch("Grabber.core.minigames.invalidate_user_cache", AsyncMock()):
