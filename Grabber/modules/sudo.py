...
async def addsudo_handler(_, message: types.Message):
    if len(message.command) < 2 or not message.command[1].isdigit():
        return await message.reply_text("❌ Provide a User ID.", parse_mode=ParseMode.MARKDOWN)

    target_id = int(message.command[1])
    if await sudo_collection.find_one({"user_id": target_id}):
        return await message.reply_text("❗ This user is already a sudo user.", parse_mode=ParseMode.MARKDOWN)

    await sudo_collection.insert_one({"user_id": target_id})
    if target_id not in sudo_users:
        sudo_users.append(target_id)
        
    await message.reply_text(f"✅ User `{target_id}` added to sudo list.", parse_mode=ParseMode.MARKDOWN)
...
async def rmsudo_handler(_, message: types.Message):
    ...
    if res.deleted_count > 0:
        ...
        await message.reply_text(f"✅ User `{target_id}` removed from sudo list.", parse_mode=ParseMode.MARKDOWN)
    else:
        await message.reply_text("❌ User not found in sudo list.", parse_mode=ParseMode.MARKDOWN)
...
async def sudolist_handler(_, message: types.Message):
    ...
    if not sudos:
        return await message.reply_text("Empty sudo list.", parse_mode=ParseMode.MARKDOWN)
