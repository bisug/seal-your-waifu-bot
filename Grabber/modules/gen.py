...
async def waifugen(_, message: types.Message):
    if len(message.command) != 3:
        await message.reply_text("Invalid usage. Usage: `/waifugen <waifu_id> <quantity>`", parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    try:
        await app.send_message(chat_id=SUPPORT_ID, text=log_text, parse_mode=ParseMode.MARKDOWN_V2)
...
async def claimwaifu(_, message: types.Message):
    if len(message.command) < 2:
        await message.reply_text("Usage: `/claimwaifu <code>`", parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    if details:
        if details['quantity'] > 0:
            ...
            await message.reply_photo(photo=waifu['img_url'], caption=response_text, parse_mode=ParseMode.MARKDOWN_V2)
