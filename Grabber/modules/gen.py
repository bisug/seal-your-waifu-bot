...
async def waifugen(_, message: types.Message):
    ...
    response_text = (
        ...
        f"Generated by: {f"[{md_escape(message.from_user.first_name)}](tg://user?id={message.from_user.id})"}"
    )

    await message.reply_text(response_text, parse_mode=ParseMode.MARKDOWN)

    log_text = (
        f"Waifu generated by {f"[{md_escape(message.from_user.first_name)}](tg://user?id={message.from_user.id})"} (ID: `{message.from_user.id}`):\n"
        f"Code: `{code}`\n"
        f"Name: {waifu['name']}\n"
        f"Anime: {waifu['anime']}\n"
...
async def claimwaifu(_, message: types.Message):
    ...
    if details:
        if details['quantity'] > 0:
            ...
            response_text = (
                fr"Congratulations {f"[{md_escape(message.from_user.first_name)}](tg://user?id={message.from_user.id})"}! You have received a new waifu for your harem 💍\!\n"
                f"Name: {md_escape(waifu['name'])}\n"
            ...
            log_text = (
                f"Waifu claimed by {f"[{md_escape(message.from_user.first_name)}](tg://user?id={message.from_user.id})"} (ID: `{user_id}`):\n"
                f"Code: `{code}`\n"
                f"Name: {waifu['name']}\n"
                f"Rarity: {waifu['rarity']}\n"
