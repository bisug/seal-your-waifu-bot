...
async def hunt(update: Update, context: CallbackContext):
    ...
    if cooldown_active:
        await update.message.reply_text(f"⏳ Please wait {seconds_left}s before hunting again.")
        return
    ...
    msg = await update.message.reply_text(f"🦊 {pet['name']} is going hunting...")
...
async def hatch(update: Update, context: CallbackContext):
    ...
    eggs = user.get("eggs", [])
    if not eggs:
        await update.message.reply_text("❌ You don’t have any eggs to hatch!")
        return

    egg = eggs[0]
    await user_collection.update_one({"id": user_id}, {"$pull": {"eggs": egg}}, upsert=True)

    msg = await update.message.reply_text("🥚 Hatching your egg...")
    await asyncio.sleep(1)
...
# /eggs command
async def eggs(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    user = await user_collection.find_one({"id": user_id}) or {}
    egg_count = len(user.get("eggs", []))
    await update.message.reply_text(f"🥚 You have <b>{egg_count}</b> eggs.", parse_mode="HTML")


# /claimegg command
async def claimegg(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    user = await user_collection.find_one({"id": user_id}) or {}

    if user.get("claimed_egg", False):
        await update.message.reply_text("❌ You've already claimed your free egg.")
        return
    ...
    await update.message.reply_text("✅ You claimed 1 free egg! Use /hatch to open it.")


# Register handlers
application.add_handler(CommandHandler("hunt", hunt))
application.add_handler(CommandHandler("hatch", hatch))
application.add_handler(CommandHandler("eggs", eggs))
application.add_handler(CommandHandler("claimegg", claimegg))
