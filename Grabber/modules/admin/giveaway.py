...
async def process_core_claim(client, user, code: str):
    ...
    try:
        await client.send_message(chat_id=MAIN_GROUP_ID, text=log_text, parse_mode=ParseMode.HTML)
...
async def waifugen(_, message: types.Message):
    ...
    try:
        ...
        return await message.reply_text("Invalid quantity. Please enter a number.", parse_mode=ParseMode.HTML)
    ...
    if not waifu:
        return await message.reply_text("Invalid waifu ID.", parse_mode=ParseMode.HTML)
    ...
    await message.reply_text(response_text, parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    ...
    try:
        await app.send_message(chat_id=MAIN_GROUP_ID, text=log_text, parse_mode=ParseMode.HTML)
    except Exception as e:
...
async def claimwaifu(_, message: types.Message):
    ...
    if not success:
        return await message.reply_text(result, parse_mode=ParseMode.HTML)
...
async def drop_waifu(_, message: types.Message):
    ...
    if len(message.command) != 3:
        return await message.reply_text("Usage: <code>/drop &lt;waifu_id&gt; &lt;quantity&gt;</code>", parse_mode=ParseMode.HTML)

    try:
        waifu_id = message.command[1]
        quantity = int(message.command[2])
    except ValueError:
        return await message.reply_text("Quantity must be a number.", parse_mode=ParseMode.HTML)
    ...
    await reply_media_dynamic(message, waifu['img_url'], caption=caption, reply_markup=markup, parse_mode=ParseMode.HTML)
