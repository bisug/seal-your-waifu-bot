...
async def waifugen(_, message: types.Message):
    if len(message.command) != 3:
        await message.reply_text("Invalid usage. Usage: <code>/waifugen &lt;waifu_id&gt; &lt;quantity&gt;</code>", parse_mode=ParseMode.HTML)
        return
    ...
    response_text = (
        f"Generated waifu:\n<code>{code}</code>\n\n"
        f"Name: {html_escape(waifu['name'])}\nRarity: {html_escape(waifu['rarity'])}\nQuantity: {quantity}\n"
        f'Generated by: <a href="tg://user?id={message.from_user.id}">{html_escape(message.from_user.first_name)}</a>'
    )

    await message.reply_text(response_text, parse_mode=ParseMode.HTML)

    log_text = (
        f'Waifu generated by <a href="tg://user?id={message.from_user.id}">{html_escape(message.from_user.first_name)}</a> (ID: <code>{message.from_user.id}</code>):\n'
    ...
    try:
        await app.send_message(chat_id=SUPPORT_ID, text=log_text, parse_mode=ParseMode.HTML)
...
async def claimwaifu(_, message: types.Message):
    if len(message.command) < 2:
        await message.reply_text("Usage: <code>/claimwaifu &lt;code&gt;</code>", parse_mode=ParseMode.HTML)
        return
    ...
    if details:
        if details['quantity'] > 0:
            ...
            response_text = (
                f'Congratulations <a href="tg://user?id={message.from_user.id}">{html_escape(message.from_user.first_name)}</a>! You have received a new waifu for your harem 💍!\n'
                f"Name: {html_escape(waifu['name'])}\n"
